...
class CustomTextLoader:
    ...
    async def text_from_csv(self, file, file_name=None):
        ...
        hdbscan_model = hdbscan.HDBSCAN(min_cluster_size=5, min_samples=2, metric='euclidean', cluster_selection_epsilon=0.1, prediction_data=True)

        # Create a BERTopic model with custom embeddings, UMAP, and HDBSCAN clustering
        topic_model = BERTopic(embedding_model=openai_embeddings, umap_model=umap_model, hdbscan_model=hdbscan_model, min_topic_size=2, calculate_probabilities=True)
    ...
    try:
        ...
        async with aiofiles.open(file_path, 'rb') as file:
            ...
            if file_extension == '.csv':
                document_instance, documents = await loader.load(file_stream, file_type=file_extension, file_name=file_path)
...
async def embed_documents_in_batches(documents, embedding_model, batch_size=10, use_sentence_transformer=False):
    ...
    with ThreadPoolExecutor() as executor:
        if use_sentence_transformer:
            futures = [loop.run_in_executor(executor, embedding_model.encode, [doc.page_content for doc in batch], True) for batch in batches]
        else:
            futures = [loop.run_in_executor(executor, embedding_model.embed_documents, [doc.page_content for doc in batch]) for batch in batches]
...
async def process_documents(main_document_path, file_paths, save_path):
    ...
    async with aiofiles.open(main_document_path, 'rb') as main_file:
        ...
        result = await loader.load(BytesIO(file_content), file_type=os.path.splitext(main_document_path)[-1].lower(), file_name=main_document_path)
